            ...
            steps {
                script {
                    if (env.GIT_BRANCH == 'origin/main') {
                        ...
                        kubectl set image deployment/flask flask-container=callantwy/myflaskapp:v${BUILD_NUMBER} -n prod
                        '''
                    } else if (env.GIT_BRANCH == 'origin/dev') {
                        sh'''
                        kubectl apply -f . -n dev
                        kubectl set image deployment/flask flask-container=callantwy/myflaskapp:v${BUILD_NUMBER} -n dev
                        '''
                    } else {
                        sh'echo "Unrecogognised branch"'
                    }
